...
interface UploadedDocument {
  ...
  type: string;
  uploadDate: Date;
  file: File; // <== Store the actual file here
}
...
export const AITutorInterface: React.FC<AITutorInterfaceProps> = ({
  ...
  function generateModulesForSubject(subject: Subject): ModuleItem[] {
    ...
    const moduleTemplates = {
      Mathematics: [
        ...
        },
        {
          ...
          lessons: ["Limits", "Derivatives", "Integration", "Applications"],
        },
      ],
      Physics: [
        {
      ...
      History: [
        ...
        },
        {
          ...
          lessons: ["Feudalism", "Crusades", "Renaissance", "Reformation"],
        },
        {
          name: "Modern Era",
          lessons: [
      ...
      JavaScript: [
        ...
        },
        {
          ...
          lessons: ["Closures", "Promises", "Async/Await", "ES6+ Features"],
        },
      ],
      React: [
        {
          name: "Core Concepts",
          lessons: ["Components", "JSX", "Props and State", "Event Handling"],
        },
        {
          name: "Advanced React",
          lessons: [
        ...
        },
        {
          ...
          lessons: ["Routing", "Testing", "Build Tools", "Deployment"],
        },
      ],
      Python: [
        {
        ...
        },
        {
          ...
          lessons: ["NumPy", "Pandas", "Flask/Django", "Data Analysis"],
        },
      ],
    };

    const templates = moduleTemplates[
      subject.name as keyof typeof moduleTemplates
    ] || [
      {
        name: "Introduction",
        lessons: [
    ...
    return templates.map((template, moduleIndex) => ({
      ...
      isCompleted:
        moduleIndex <
        Math.floor(
          subject.completedTopics / (subject.totalTopics / templates.length)
        ),
      isLocked:
        moduleIndex >
        Math.floor(
      ...
      lessons: template.lessons.map((lessonName, lessonIndex) => ({
        ...
        name: lessonName,
        type: ["video", "text", "quiz", "assignment"][lessonIndex % 4] as
          | "video"
          | "text"
          | "quiz"
          | "assignment",
  ...
  const generateSubjectColor = (topic: string | undefined): string => {
    if (!topic || typeof topic !== "string") return "from-blue-500 to-blue-600";
  ...
  const handleGenerateCourse = async () => {
    for (const doc of uploadedDocs) {
      ...
      try {
        ...
        console.log(`Uploaded ${doc.name} successfully`, result);

        // Generate course data from the uploaded file's metadata
        const chapters = result.chapters; // Assuming the response contains 'chapters'

        // Map originalFileName to the course title
        const fileName = result.originalFileName || doc.name || "Untitled";
        const newSubject: Subject = {
          id: `custom-${Date.now()}`,
          name: fileName, // Map to the original file name as the course title
          icon: "BookOpen",
          description: `AI-generated course based on ${fileName}`,
          color: generateSubjectColor(fileName),
          totalTopics: chapters.length, // Total number of chapters
          completedTopics: 0,
          difficulty: selectedLevel as "Beginner" | "Intermediate" | "Advanced",
        };

        // Create modules and lessons from the chapters
        const modules = chapters.map((chapter: any, index: number) => ({
          id: `${newSubject.id}-module-${index}`,
          name: chapter.chapterName, // Map chapterName to module name
          isCompleted: false,
          isLocked: false,
          lessons: [
            {
              id: `${newSubject.id}-module-${index}-lesson-1`,
              name: chapter.chapterName, // Use chapterName as lesson title
              type: "text", // Assuming text type here, update based on actual content type
              duration: 30,
              isCompleted: false,
              isLocked: false,
              content: chapter.content, // Attach content from the chapter
            },
          ],
        }));

        console.log("Generated new subject:", handleGenerateCourse);

        // Map chapters into the course structure
        onCreateSubject(newSubject);

        // Optionally, you can map the modules to the UI
        setCourseTree((prev) => [
          ...prev,
          {
            id: newSubject.id,
            ...
            completedTopics: 0,
            isExpanded: true,
            modules: modules, // Use the generated modules
            subject: newSubject,
          },
        ]);
  ...
  const handleFileUpload = (files: FileList | null) => {
    ...
    if (newFiles.length > availableSlots) {
      alert(
        `You can only add ${availableSlots} more items. Maximum total is 4 files and links combined.`
      );
      return;
    }
    ...
    for (const file of newFiles) {
      ...
      if (currentTotalSize + fileSizeInMB > 100) {
        alert(
          `Adding "${
            file.name
          }" would exceed the 100MB total limit. Current total: ${currentTotalSize.toFixed(
            1
          )}MB`
        );
        continue;
  ...
  return (
    ...
    >
      ...
      <div className="flex">
        <div
          className={`w-80 min-h-screen border-r transition-colors duration-300 ${
            darkMode
          ...
          <div className="p-6">
            ...
            <div className="mb-6 relative">
              ...
              {showSearchResults && searchResults.length > 0 && (
                <div
                  className={`absolute top-full left-0 right-0 mt-1 border rounded-lg shadow-lg z-50 max-h-64 overflow-y-auto transition-colors duration-300 ${
                    darkMode
              ...
              {showSearchResults &&
                ...
                searchTerm && (
                  <div
                    className={`absolute top-full left-0 right-0 mt-1 border rounded-lg shadow-lg z-50 p-4 text-center transition-colors duration-300 ${
                      darkMode
                        ? "bg-gray-800 border-gray-600"
                        : "bg-white border-gray-200"
                    }`}
                  >
                    ...
                    >
                      Try searching for a different term or browse your courses
                      below
                    </p>
                  </div>
                )}
            ...
            {(uploadedDocs.length > 0 || addedLinks.length > 0) && (
              <div className="mb-8">
                ...
                >
                  Your Resources ({getTotalItemsCount()}/4)
                </h3>
                        ...
                        <div className="ml-4 space-y-1">
                          {course.modules.map((module) => (
                            <div key={module.id} className="space-y-1">
                              <button
                                onClick={(e) =>
                                  handleModuleClick(course, module, e)
                                }
                                ...
                                className={`w-full flex items-center space-x-2 p-1.5 rounded text-xs transition-colors ${
                                  ...
                                  module.isCompleted
                                    ...
                                    ? "text-gray-400"
                                    : darkMode
                                    ? "text-gray-400 hover:bg-blue-900 hover:border-blue-700"
                                    : "text-gray-600"
                                }`}
                              >
                                <div
                                    ...
                                    <button
                                      ...
                                      className={`w-full flex items-center space-x-2 p-1 text-xs transition-colors ${
                                        ...
                                        lesson.isCompleted
                                          ...
                                          ? "text-gray-400"
                                          : darkMode
                                          ? "text-gray-500 hover:bg-blue-900 hover:border-blue-700"
                                          : "text-gray-500"
                                      }`}
                                    >
            ...
            >
              ...
              <div
                className={`w-full bg-yellow-200 rounded-full h-1 mb-2 ${
        ...
        <div className="flex-1">
          {/* Top Banner */}
          <div
            className={`border-b px-8 py-4 flex items-center justify-between transition-colors duration-300 ${
              darkMode
                ? "bg-gray-800 border-gray-700"
                : "bg-white border-gray-200"
            }`}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          ...
          >
            <div className="flex items-center justify-between mb-6">
              ...
              >
                Add Resources ({getTotalItemsCount()}/4)
              </h3>
              <button
            ...
            >
              <div className="flex items-start space-x-2">
                ...
                <div className="text-xs">
                  ...
                  <ul
                    className={`mt-1 space-y-1 transition-colors duration-300 ${
                      darkMode ? "text-gray-400" : "text-gray-600"
                    }`}
                  >
